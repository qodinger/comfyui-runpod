...
def create_auth_middleware(api_key_manager: APIKeyManager, require_auth: bool = False):
    ...
    async def auth_middleware(request: web.Request, handler):
        ...
        # Fixed Bug 5: Protect API key management endpoints (/api/keys, /keys)
        # These endpoints must be protected when require_auth is enabled
        is_api_key_endpoint = (path.startswith('/api/keys') or path.startswith('/keys') or
                              path.startswith('/api/usage') or path.startswith('/usage'))

        # For API endpoints, check authentication
        if path.startswith('/api/') or path.startswith('/prompt') or path.startswith('/queue') or is_api_key_endpoint:
            if require_auth:
                if not api_key:
